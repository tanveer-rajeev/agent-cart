...
> Full test suite: [`src/test/`](./src/test/) — each service has its own test module mirroring the production package structure.
...
- Ollama installed locally (`ollama pull mistral`)
...
This starts Kafka, Zookeeper, PostgreSQL (one instance per service), and all microservices.
