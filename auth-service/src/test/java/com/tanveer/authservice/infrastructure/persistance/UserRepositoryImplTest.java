...
class UserRepositoryImplTest {
    ...
    void shouldUpdateUser() throws CustomException {
        ...
        when(userJpaRepository.findByEmail(user.getEmail())).thenReturn(Optional.empty());

        when(userJpaRepository.findById("1")).thenReturn(Optional.of(entity));

        when(userJpaRepository.saveAndFlush(entity)).thenReturn(entity);
        ...
        verify(userJpaRepository).findByEmail("test2@gmail.com");
